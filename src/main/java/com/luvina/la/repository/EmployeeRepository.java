...
public interface EmployeeRepository extends JpaRepository<Employee, Long> {
       ...
       @Query(value = "SELECT e.employee_id AS employeeId, e.employee_name AS employeeName, e.employee_birth_date AS employeeBirthDate, "
                     ...
                     "WHERE e.employee_id = :employeeId AND e.employee_role = 1 " +
        ...
        * @return Trang kết quả chứa danh sách EmployeeDTO
        */
       @Query(value = "SELECT e.employee_id AS employeeId, e.employee_name AS employeeName, e.employee_birth_date AS employeeBirthDate, "
                     +
                     ...
                     "AND e.employee_role = 1", countQuery = "SELECT COUNT(*) FROM employees e " +
                                    "LEFT JOIN departments d ON e.department_id = d.department_id " +
                                    "WHERE (:employeeName IS NULL OR e.employee_name LIKE CONCAT('%', :employeeName, '%') ESCAPE '\\\\') "
                                    +
                                    "AND (:departmentId IS NULL OR e.department_id = :departmentId) " +
                                    "AND e.employee_role = 1", nativeQuery = true)
